...
export function Mcp() {
  return (
    <main style={{ maxWidth: 820, margin: "0 auto", padding: "56px 24px 96px" }}>
      <a href="/" className="hero" style={{ fontSize: 24 }}>
        Lyra
      </a>
      ...
      <p style={{ color: "var(--ink-2)", fontSize: 14, margin: "12px 0 0", maxWidth: 620, lineHeight: 1.6 }}>
      ...
      <Block title="Install">
        <Code text={CONFIG} />
        <p style={{ fontSize: "var(--t-body)", color: "var(--ink-2)", margin: "12px 0 0" }}>
      ...
      <Block title="What it will not do">
        <p style={{ fontSize: "var(--t-data)", color: "var(--ink-2)", margin: 0, lineHeight: 1.65 }}>
      ...
      <footer style={{ marginTop: 56, paddingTop: 20, borderTop: "1px solid var(--rule)", fontSize: "var(--t-body)", color: "var(--ink-2)", display: "flex", gap: 20 }}>
...
function Tool({ name, desc }: { name: string; desc: string }) {
  return (
    <div style={{ padding: "9px 0", borderBottom: "1px solid var(--rule)" }}>
      <div className="mono" style={{ fontSize: "var(--t-data)", }}>{name}</div>
      <div style={{ fontSize: "var(--t-body)", color: "var(--ink-2)", marginTop: 3 }}>{desc}</div>
    </div>
  );
}
