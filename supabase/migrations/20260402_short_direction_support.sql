...
begin
  ...
  v_detail := concat(initcap(p_direction), ' ', v_quantity::text, ' ', p_symbol, ' @ ', p_price::text);
...
declare
  ...
  v_activity public.workspace_activity;
  v_detail text;
  v_previous_stop numeric;
  v_previous_take numeric;
  v_title text;
  v_type text;
begin
  if p_stop_loss is not null and p_stop_loss <= 0 then
  ...
  if v_position.direction = 'long' then
    if p_stop_loss is not null and p_stop_loss >= v_position.entry_price then
      raise exception 'Stop loss must be below the entry price for a long setup';
    end if;

    if p_take_profit is not null and p_take_profit <= v_position.entry_price then
      raise exception 'Take profit must be above the entry price for a long setup';
    end if;
  else
    if p_stop_loss is not null and p_stop_loss <= v_position.entry_price then
      raise exception 'Stop loss must be above the entry price for a short setup';
    end if;

    if p_take_profit is not null and p_take_profit >= v_position.entry_price then
      raise exception 'Take profit must be below the entry price for a short setup';
    end if;
  end if;

  v_previous_stop := v_position.stop_loss;
  v_previous_take := v_position.take_profit;
  ...
  if p_stop_loss is distinct from v_previous_stop and not (p_take_profit is distinct from v_previous_take) then
    v_title := 'Stop loss updated';
    v_type := 'position.levels.updated.stop_loss';
  elsif p_take_profit is distinct from v_previous_take and not (p_stop_loss is distinct from v_previous_stop) then
    v_title := 'Take profit updated';
    v_type := 'position.levels.updated.take_profit';
  else
    v_title := 'Trade setup updated';
    v_type := 'position.levels.updated';
  end if;
...
begin
  ...
  v_return_amount := case
    when v_position.direction = 'short' then (v_position.entry_price * p_quantity) + v_realized
    else v_notional
  end;
